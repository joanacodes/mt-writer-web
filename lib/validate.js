...
export function validate(text, row, lang, paths, opts = {}) {
  const problems = [];
  const [fm, body] = frontMatter(text);
  if (!fm) return { problems: ['no front matter'], words: 0 };
  for (const k of ['title', 'description', 'translationKey', 'type', 'date', 'categories', 'tags', 'summary', ...(opts.imagePrompts ? ['image', 'imageAlt', 'imagePrompt'] : [])])
