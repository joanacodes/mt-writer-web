import { db, log, settings } from './db';
import { submitBatch, anthropicParams, REASONING } from './providers';
...
export async function submitBatchFor({ ids, mode, model, chain = '' }) {
  ...
  for (const row of rows || []) {
    ...
    const content = mode === 'en' ? userEn(row, today, st) : userFr(row, enBy[row.id], paths, today, st);
    requests.push({ custom_id, params: anthropicParams({ model, system, user: content, maxTokens: 16000, mode: st.reasoning || 'thinking_off' }) });
  }
